...
class PreSellout(Transformation):
    ...
    def transformation_atacadao_presellout(
        self, df_atacadao_sales: DataFrame
    ) -> DataFrame:

        # Split the column 'MULT' into two new columns 'CXA' and 'PACK' using " X " as the delimiter
        df_atacadao_transformed = df_atacadao_sales.withColumn(
            "CXA", split(df_atacadao_sales["MULT"], " X ").getItem(0)
        ).withColumn("PACK", split(df_atacadao_sales["MULT"], " X ").getItem(1))

        # Apply transformations and standardizations to the DataFrame
        df_atacadao_transformed = df_atacadao_transformed.select(
            coalesce(trim(df_atacadao_transformed.CNPJ), lit("0"))
            .cast(LongType())
            .alias("CNPJ_PDV"),
            to_date(
                coalesce(
                    to_date((df_atacadao_transformed.DATA), "dd/MM/yyyy"),
                    lit("19000101"),
                ),
                "yyyy-MM-dd",
            ).alias("DATA"),
            translate(
                coalesce(df_atacadao_transformed.QTDE_VENDA_UNIDADE, lit("0")), ",", "."
            )
            ...
            upper(
                coalesce(df_atacadao_transformed.DESCRICAO, lit("NAO INFORMADO"))
            ).alias("PRODUCT_SOURCE_NAME"),
            lit("NAO INFORMADO").alias("BRAND"),
            coalesce(
                df_atacadao_transformed.CODIGO_BARRA.cast(LongType()), lit(0)
            ).alias("PRODUCT_SOURCE_CODE"),
            coalesce(col("CODIGO_BARRA"), lit("0"))
            .cast(LongType())
            .alias("EAN_SOURCE_CODE"),
            lit("NAO INFORMADO").alias("PDV_COMPANY_NAME"),
            lit("NAO INFORMADO").alias("PDV_TYPE"),
            lit("NAO INFORMADO").alias("ID_PDV_ECOMMERCE"),
            translate(
                coalesce(df_atacadao_transformed.ESTOQUE_BRUTO, lit("0")), ",", "."
            )
            .cast(DecimalType(38, 6))
        ...
        df_atacadao_transformed = df_atacadao_transformed.withColumn(
        ...
        if date.today() != first_day_of_the_month:
            df_atacadao_transformed = df_atacadao_transformed.filter(
                col("DATA").between(start_date, end_date)
            )

        # Aggregate the data by the specified columns and sum the relevant metrics
        df_atacadao_transformed_group = df_atacadao_transformed.groupBy(
            "CATEGORY",
            "BRAND",
            "PRODUCT_SOURCE_CODE",
            "EAN_SOURCE_CODE",
    ...
    def select_columns_atacadao_presellout(
        self, df_atacadao_sales: DataFrame
    ) -> DataFrame:

        # Select specific columns from the DataFrame and add new columns with constant values
        df_atacadao_sales_selected = df_atacadao_sales.select(
            df_atacadao_sales.CATEGORY,
            lit("AMBEV").alias(
                "COMPANY"
            ),  # Add a new column 'COMPANY' with a constant value 'AMBEV'
            ...
            lit("ATACADAO").alias(
                "SOURCE_NAME"
            ),  # Add a new column 'SOURCE_NAME' with a constant value 'ATACADAO'
            lit("ATACADAO").alias(
                "SUBSOURCE_NAME"
            ),  # Add a new column 'SUBSOURCE_NAME' with a constant value 'ATACADAO'
            df_atacadao_sales.CNPJ_PDV.alias("CNPJ_PDV"),
            lit("0")
            .cast(IntegerType())
            .alias("CHECKOUT"),  # Add a new column 'CHECKOUT' with a constant value '0'
        ...
        df_atacadao_sales_selected = (
            df_atacadao_sales_selected.withColumn(
                "ano_partition", year("DATA")
            )  # Extract the year from 'DATA' and add it as 'ano_partition'
            .withColumn(
                "mes_partition", month("DATA")
            )  # Extract the month from 'DATA' and add it as 'mes_partition'
            .withColumn(
                "dia_partition", dayofmonth("DATA")
            )  # Extract the day of the month from 'DATA' and add it as 'dia_partition'
        )
    ...
    def definitions(self):
        ...
        df_atacadao_transformed = self.transformation_atacadao_presellout(
            df_atacadao_sales
        )

        df_atacadao_pre_sellout = self.select_columns_atacadao_presellout(
...
# MAGIC %sql
# MAGIC SELECT DISTINCT to_date(DATA, 'dd/MM/yyyy') 
